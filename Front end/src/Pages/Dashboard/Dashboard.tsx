...
import { LayoutDashboard, Users, FileText, Settings, LogOut, Search, SlidersHorizontal, AlertCircle, BarChart as BarChartIcon } from 'lucide-react';
import { XAxis, YAxis, Tooltip, ResponsiveContainer, LineChart, Line, PieChart, Pie, Cell, BarChart, Bar } from 'recharts';
...
const MetricCard: React.FC<MetricCardProps> = ({ title, value, unit, isPrimary = false }) => (
    <div className={`${styles.metricCard} ${isPrimary ? styles.metricCardPrimary : ''}`}>
...
const VisualizationPanel: React.FC<VisualizationPanelProps> = ({ title, children, fullWidth = false }) => (
    <div className={`${styles.visualizationPanel} ${fullWidth ? styles.panelFullWidth : ''}`}>
        ...
        <div className={styles.appContainer}>
            ...
            <div className={styles.sidebar}>
                ...
                <nav className={styles.navMenu}>
                    ...
                    <div className={`${styles.navItem} ${styles.navActive}`} onClick={() => handleMenuItemClick('/dashboard')}>
                    ...
                    <div className={styles.navItem} onClick={() => handleMenuItemClick('/auditoria')}>
                        <div className={styles.navIcon}><BarChartIcon size={20} /></div>
            ...
            <div className={styles.mainContent}>
                ...
                <div className={styles.metricCardsContainer}>
                    ...
                    <MetricCard title="Tempo Médio de Resposta" value="17" unit="min" isPrimary={true} />
                </div>
                ...
                <div className={styles.dashboardGrid}>
                    ...
                    <div className={styles.dashboardRow}>
                                ...
                                <ul className={styles.turnoLegend}>
                                    {dataTurno.map((item, index) => (
                                        <li key={index} className={styles.turnoLegendItem}>
                                            <span style={{ backgroundColor: item.color }} className={styles.legendDot}></span> 
                                            {item.name} <span className={styles.legendValue}>{(item.value / 1000) + 'K'}</span>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        ...
                        <VisualizationPanel title="Ocorrências por Tipo">
                            <div style={{ width: '100%', height: 280, padding: '1rem 0' }}>
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={dataTipo} margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
                    ...
                    <VisualizationPanel title="Ocorrências por região" fullWidth={true}>
                        <div className={styles.regionLegend}>
                            <span><span style={{backgroundColor: 'var(--status-closed)'}} className={styles.legendDot}></span> Comando Operacional Metropolitano (COM)</span>
                            <span><span style={{backgroundColor: 'var(--status-in-progress)'}} className={styles.legendDot}></span> Comando Operacional do Interior I (COInter/I)</span>
                            <span><span style={{backgroundColor: '#007bff'}} className={styles.legendDot}></span> Comando Operacional do Interior II (COInter/2)</span>
                        </div>

                        <div style={{ width: '100%', height: 280 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={dataRegiao} margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
                                    <XAxis dataKey="dia" style={{ fontSize: '10px' }} />
                                    <YAxis style={{ fontSize: '10px' }} />
                                    <Tooltip />
                                    <Line type="monotone" dataKey="COM" stroke="var(--status-closed)" strokeWidth={2} dot={false} />
                                    <Line type="monotone" dataKey="COInter1" stroke="var(--status-in-progress)" strokeWidth={2} dot={false} />
                                    <Line type="monotone" dataKey="COInter2" stroke="#007bff" strokeWidth={2} dot={false} />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </VisualizationPanel>
